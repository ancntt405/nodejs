...
app.get('/baiviet', (req, res) => {
    Promise.all([
            ...
            new Promise((resolve, reject) => {
                connection.query('SELECT * FROM brands', (err, data) => {
                    if (err) return reject(err);
                    resolve(data);
                });
            }),
        ])
        .then(([categories, products, banner, brands]) => {
            res.render('baiviet', {
                title: 'Danh Mục Sản Phẩm',
            ...
            console.log(categories, products, banner, brands);
        })
        .catch(err => {
            console.error(err);
            res.status(500).send('Loi ket noi co so du lieu');
...
app.get('/', (req, res) => {
    ...
    Promise.all([
            new Promise((resolve, reject) => {
                connection.query('SELECT * FROM banners ORDER BY id DESC', (err, data) => {
                    if (err) return reject(err);
                    resolve(data);
                });
            }),
            ...
            new Promise((resolve, reject) => {
                connection.query('SELECT * FROM products ORDER BY RAND() LIMIT 6', (err, data) => {
                    if (err) return reject(err);
                    resolve(data);
                });
            }),
            new Promise((resolve, reject) => {
                connection.query('SELECT * FROM products ORDER BY RAND() LIMIT 8', (err, data) => {
                    if (err) return reject(err);
                    resolve(data);
                })
            })
